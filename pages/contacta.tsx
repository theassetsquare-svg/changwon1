...
/* ★ 2026-09-02 — <Head> 로 뒤에서 덮어쓰는 방법은 **먹지 않았다.**
   실측: 이 쪽의 canonical 이 https://j.nolcool.com/access/busan-asiad-night/ 로 나가고 있었다.
   그러면 네이버는 이 쪽을 그쪽의 사본으로 보고 절대 색인하지 않는다.
   그래서 컴포넌트에 주소를 직접 넘겨 한 곳에서만 만들게 바꿨다 [[url-one-shape-rule]]. */
export default function ContactaAdPage({ venue }: { venue: AccessVenue }) {
  const 변형 = 변형쪽들["/contacta"];
  return (
    <AccessVenuePage
      venue={venue}
      변형={변형}
